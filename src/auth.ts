...
export function initAuth0(config) {
  ...
  return {
    ...
    getAccessToken: auth0WrapperJson((req, res, auth0FnOptions) => auth0.getAccessToken(req, res, auth0FnOptions)),
    handleLogin: auth0Wrapper((req, res, auth0FnOptions) => auth0.handleLogin(req, res, auth0FnOptions)),
    handleLogout: auth0Wrapper((req, res, auth0FnOptions) => auth0.handleLogout(req, res, auth0FnOptions)),
    handleCallback: auth0Wrapper((req, res, auth0FnOptions) => auth0.handleCallback(req, res, auth0FnOptions)),
    handleProfile: auth0Wrapper((req, res, auth0FnOptions) => auth0.handleProfile(req, res, auth0FnOptions)),
        ...
        if (isAuthenticated) {
          ...
          if (opts?.load && typeof opts?.load === 'function') {
            ...
            if (loadResult && typeof (loadResult as any).then === 'function') {
              ...
              // Synchronous load()
              return { ...loadResult, props: { ...(loadResult as LoadOutput).props, user, isAuthenticated }}
            } else {
              // If user's load() function returned nothing, they intend to fall through, so we shouldn't populate user props
              return loadResult;
            }
